...
export default function ProductCategory() {
  ...
  const filteredProducts = useMemo(()=> selectedCategoryProducts.filter((product) => {
    const passDiscountFilter = selectedDiscountValues.length === 0 || selectedDiscountValues.some((discount) => parseInt(product.discountPercentage) >= parseInt(discount.replace("discount", "")));
    const passBrandFilter = selectedBrands.length === 0 || selectedBrands.includes(product.brand);
    const passRatingFilter = selectedRatings.length === 0 || selectedRatings.includes(String(parseInt(product.rating)));
    return passDiscountFilter && passBrandFilter && passRatingFilter;
  }),[selectedBrands,selectedCategoryProducts,selectedRatings,selectedDiscountValues])
  ...
  //   return discountId.some(ids=> parseInt(product.discountPercentage) > ids )
  // })

  // const filterByRating = selectedCategoryProducts.filter((product)=>{
  //   return ratingId.some(ids=>parseInt(product.rating)==ids)
  // })
     ...
     <div className={style.filterDiv} >
      ...
      <div className={style.sidebarContainer}>
        <div className={style.sidebar}>
          ...
          {/* <div>
        {selectedBrands.join(", ")} <button>X</button>
      </div> */}
          ...
          <select multiple  value={filters.selectedRatings} onChange={handleRatingChange}>
         ...
         { uniqueBrands.length > 1 && <div >
            <h2>BRAND</h2>
            
            <div className={style.brands} >
     
                  <select multiple  value={filters.selectedBrands} onChange={handleBrandChange} >
      ...
      <div className={style.sortDiv} >
        ...
        <button  onClick={()=>handleSortChange(event)} value="priceLowToHigh" className={sortOption === 'priceLowToHigh' ? `${style.sortBtnLow} ${style.sortBtn} ` : style.sortBtn } >Price--Low to High</button>
  
        <button onClick={()=>handleSortChange(event)} value="priceHighToLow"  className={sortOption === 'priceHighToLow' ? `${style.sortBtnHigh} ${style.sortBtn} ` : style.sortBtn } >Price--High to Low</button>
      </div>
  ...
  return (
    <div className={style.box}>
      ...
      <div className={style.detailsProductInfo}>
       ...
       { product.brand && <p style={{ color: "#524C42" }}>Brand: {product.brand}</p>}
      ...
      <div className={style.priceDiv}>
        ...
        {product.price > 10 && <p>Free delivery</p>}
        {product.discountPercentage > 12 && (
          <h4 className={style.LowestPriceMonth}>Lowest Price in this Month</h4>
        )}
        {product.stock < 50 && <p className={style.onlyFew}>only few left</p>}
      </div>
    </div>
  );
};
